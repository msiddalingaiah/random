...
/*
 * $Id: ControlFlowGraph.java,v 1.1 2008/04/30 04:23:47 madhu Exp $
 */
...
public class ControlFlowGraph {
	...
	private HashMap<String,SSAStack> renumberMap;
	...
	*/
	private void doComputeDominance() {
		...
		while (changed) {
			...
			while (i >= 0) {
				Node b = postOrderList.get(i--);
				if (b == startBlock) {
					continue;
				}
				Iterator<Node> ip = b.getPredecessors().iterator();
				if (!ip.hasNext()) {
	...
	*/
	private void computeDominanceFrontier() {
		...
		for (int i=0; i<n; i+=1) {
			...
			List<Node> predList = b.getPredecessors();
			if (predList.size() >= 2) {
				Iterator<Node> it = predList.iterator();
				while (it.hasNext()) {
	...
	public void constructSSA() {
		computeDominance();
		placePhiFunctions();
		renumberMap = new HashMap<String,SSAStack>();
		renameVariables(getStartBlock());
	}
	...
	private void placePhiFunctions() {
		...
		while (b != null) {
			...
			for (int i=0; i<defList.size(); i+=1) {
				...
				List<Node> df = b.getDominanceFrontier();
				Iterator<Node> it = df.iterator();
				while (it.hasNext()) {
...
//					dfb.add(new PhiAssignQuad(def));
	...
	private void renameVariables(Block block) {
		...
		List<Node> list = block.getSuccessors();
		int n = list.size();
		for (int i = 0; i < n; i++) {
			Block b = (Block) list.get(i);
			rewritePhiParams(b);
		}
		Iterator<Node> it = block.getDominatedBlocks().iterator();
		while (it.hasNext()) {
	...
	private void doRenameVariables(Block block) {
		Iterator<SGCTree> it = block.getTrees().iterator();
		while (it.hasNext()) {
			SGCTree q = it.next();
			renameRHS(q);
			if (q.getType() == SyntaxParser.ASSIGN) {
				SGCTree lhs = (SGCTree) q.getChild(0);
				Token token = lhs.getToken();
				SSAStack st = getStack(token.getText());
				token.setText(st.getNewVariable());
			}
		}
	}

	private void renameRHS(SGCTree q) {
		if (q.getType() == SyntaxLexer.IDENT) {
			Token token = q.getToken();
			SSAStack st = getStack(token.getText());
			if (st != null) {
				token.setText(st.peek());
			}
		}
		int n = q.getChildCount();
		for (int i = 0; i < n; i += 1) {
			SGCTree child = q.getSGCChild(i);
			renameRHS(child);
		}
	}
...
//			}
	...
	private SSAStack getStack(String name) {
		SSAStack st = renumberMap.get(name);
		if (st == null) {
...
//			}
	...
	public void printBlocks() {
		...
		for (int i=n-1; i >= 0; i-=1) {
			...
			System.out.print("; DF:");
			Iterator<Node> it = b.getDominanceFrontier().iterator();
			while (it.hasNext()) {
 ...
 */
	...
	private static void printBlocks(Node[] b) {
		for (int i=0; i < b.length; i+=1) {
			...
			System.out.print("; DF:");
			Iterator<Node> it = b[i].getDominanceFrontier().iterator();
			while (it.hasNext()) {
				System.out.print(" " + it.next());
			}
			System.out.print("; Dom blocks:");
