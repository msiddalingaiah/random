...
import static com.madhu.throttle.Units.*;

public class FlowBenchIntegrator {
	public static final double GAS_CONSTANT_SI = 8.314472;
	public static final double MSL_PRESSURE_PA = 1.01325e5;
	public static final double MSL_PRESSURE_PSI = 14.696;
	/**
	 ...
	 */
	public void setThrottle(double angle) {
		angle += closedAngle;
	...
	public double[] rungeKutta4(double tmax, int n, double p) {
		double dt = tmax / n;
		double dt2 = 0.5 * dt;
		double[] data = new double[n];
		double t = 0;
	...
	public void computeIntakeStroke(double po) {
		...
		int n = 1000;
		manifoldPressure = rungeKutta4(tmax, n, po);
		minPressure = po;
	...
	private static void computeHP() {
		...
		fb.setThrottle(75);
		System.out.printf("RPM\tHP\tTorqe (ft-lb)%n");
		for (int rpm = 1500; rpm <= 9000; rpm += 250) {
			fb.setRPM(rpm);
			fb.computeIntakeStroke(po);
			double mols = fb.getAirAmount();
			double grams_hr = mols * 28.97 * rpm * 30 / afr;
			double hp = grams_hr / (bsfc * 0.746);
			System.out.printf("%d\t%f\t%f%n", rpm, hp, hp*5250/rpm);
		}
	}
